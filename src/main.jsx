...
function App(){
  ...
  return <>
    ...
    <nav>{[['dashboard','Übersicht'],['gear','Ausrüstung'],['lists','Packlisten'],['wishlist','Wunschliste'],['compare','Vergleich'],['print','Drucken'],['import','CSV-Import']].map(([id,label])=><button key={id} className={view===id?'active':''} onClick={()=>setView(id)}>{label}</button>)}</nav>
    <main>
      ...
      {view==='print'&&<PrintView gear={gear}/>} 
...
function PrintView({gear}){
 const [q,setQ]=useState(''),[category,setCategory]=useState(''),[location,setLocation]=useState(''),[sort,setSort]=useState('name-asc')
 const categories=[...new Set(gear.map(g=>g.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'de'))
 const locations=[...new Set(gear.map(g=>g.location).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'de'))
 const items=useMemo(()=>{
   const needle=q.trim().toLowerCase()
   const list=gear.filter(g=>(!needle||[g.name,g.brand,g.notes,g.category,g.location].some(v=>String(v||'').toLowerCase().includes(needle)))&&(!category||g.category===category)&&(!location||g.location===location))
   return [...list].sort((a,b)=>{
     if(sort==='name-desc')return String(b.name||'').localeCompare(String(a.name||''),'de',{sensitivity:'base'})
     if(sort==='category-asc')return String(a.category||'').localeCompare(String(b.category||''),'de',{sensitivity:'base'})||String(a.name||'').localeCompare(String(b.name||''),'de',{sensitivity:'base'})
     if(sort==='location-asc')return String(a.location||'').localeCompare(String(b.location||''),'de',{sensitivity:'base'})||String(a.name||'').localeCompare(String(b.name||''),'de',{sensitivity:'base'})
     return String(a.name||'').localeCompare(String(b.name||''),'de',{sensitivity:'base'})
   })
 },[gear,q,category,location,sort])
 const totalQty=items.reduce((s,i)=>s+Number(i.quantity||0),0)
 const totalWeight=items.reduce((s,i)=>s+Number(i.weight_g||0)*Number(i.quantity||0),0)
 return <section className="card print-register">
   <div className="print-register-toolbar no-print"><div><h2>Artikelliste drucken</h2><p className="muted">Filter festlegen, Vorschau kontrollieren und anschliessend drucken.</p></div><button type="button" className="primary print-button" onClick={()=>window.print()} disabled={!items.length}>Drucken</button></div>
   <div className="filterbar gear-filterbar no-print"><input className="search" placeholder="Artikel, Hersteller oder Notizen suchen" value={q} onChange={e=>setQ(e.target.value)}/><select value={category} onChange={e=>setCategory(e.target.value)}><option value="">Alle Kategorien</option>{categories.map(c=><option key={c}>{c}</option>)}</select><select value={location} onChange={e=>setLocation(e.target.value)}><option value="">Alle Lagerplätze</option>{locations.map(l=><option key={l}>{l}</option>)}</select><select value={sort} onChange={e=>setSort(e.target.value)}><option value="name-asc">Artikel A–Z</option><option value="name-desc">Artikel Z–A</option><option value="category-asc">Kategorie A–Z</option><option value="location-asc">Lagerplatz A–Z</option></select><button type="button" className="secondary" onClick={()=>{setQ('');setCategory('');setLocation('');setSort('name-asc')}}>Filter zurücksetzen</button></div>
   <div className="print-heading"><div><h1>PackLager – Artikelliste</h1><p>{items.length} gefilterte Artikel</p></div><div className="print-summary"><span><b>{items.length}</b> Artikel</span><span><b>{totalQty}</b> Stück</span><span><b>{weight(totalWeight)}</b> Gesamtgewicht</span></div></div>
   {items.length?<div className="print-grid">{items.map(item=><article className="print-item" key={item.id}><div className="print-item-head"><h2>{item.name}</h2><span>{item.quantity} Stück</span></div><dl><div><dt>Kategorie</dt><dd>{item.category||'–'}</dd></div><div><dt>Lagerplatz</dt><dd>{item.location||'–'}</dd></div><div><dt>Einzelgewicht</dt><dd>{weight(item.weight_g)}</dd></div><div><dt>Gesamtgewicht</dt><dd>{weight(Number(item.weight_g||0)*Number(item.quantity||0))}</dd></div>{item.brand&&<div><dt>Hersteller</dt><dd>{item.brand}</dd></div>}{Number(item.price_chf)>0&&<div><dt>Preis</dt><dd>{money(item.price_chf)}</dd></div>}</dl>{item.notes&&<p className="print-notes">{item.notes}</p>}</article>)}</div>:<Empty/>}
 </section>
}
