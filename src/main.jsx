...
function GearView({gear,refreshAll,setMessage}){
 const [form,setForm]=useState(emptyGear);const [editId,setEditId]=useState(null);const [q,setQ]=useState('')
 const filtered=gear.filter(g=>`${g.name} ${g.category} ${g.location} ${g.brand}`.toLowerCase().includes(q.toLowerCase()))
...
function CompareView({gear,lists,listItems}){
 const [a,setA]=useState(null),[b,setB]=useState(null),[la,setLa]=useState(null),[lb,setLb]=useState(null)
 const listWeight=id=>listItems.filter(i=>i.pack_list_id===id).reduce((s,i)=>s+(gear.find(g=>g.id===i.gear_id)?.weight_g||0)*i.quantity,0)
 return <div className="stack"><section className="card"><h2>Artikel vergleichen</h2><p className="muted">Artikel aus der Liste unten in die beiden Felder ziehen.</p><DropCompare title="Artikel A" item={gear.find(g=>g.id===a)} onDrop={setA} type="gearId"/><DropCompare title="Artikel B" item={gear.find(g=>g.id===b)} onDrop={setB} type="gearId"/>{a&&b&&<Result a={gear.find(g=>g.id===a).weight_g} b={gear.find(g=>g.id===b).weight_g} an={gear.find(g=>g.id===a).name} bn={gear.find(g=>g.id===b).name}/>}<div className="drag-grid">{gear.map(g=><div className="drag-item" draggable onDragStart={e=>e.dataTransfer.setData('gearId',g.id)} key={g.id}><strong>{g.name}</strong><small>{weight(g.weight_g)} · {g.location||'Kein Lagerplatz'}</small></div>)}</div></section><section className="card"><h2>Packlisten vergleichen</h2><p className="muted">Packlisten in die beiden Felder ziehen.</p><DropCompare title="Packliste A" item={lists.find(l=>l.id===la)} onDrop={setLa} type="listId" displayWeight={la?listWeight(la):null}/><DropCompare title="Packliste B" item={lists.find(l=>l.id===lb)} onDrop={setLb} type="listId" displayWeight={lb?listWeight(lb):null}/>{la&&lb&&<Result a={listWeight(la)} b={listWeight(lb)} an={lists.find(l=>l.id===la).name} bn={lists.find(l=>l.id===lb).name}/>}<div className="drag-grid">{lists.map(l=><div className="drag-item" draggable onDragStart={e=>e.dataTransfer.setData('listId',l.id)} key={l.id}><strong>{l.name}</strong><small>{weight(listWeight(l))}</small></div>)}</div></section></div>
}
function DropCompare({title,item,onDrop,type,displayWeight}){return <div className="compare-drop" onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('drop')}} onDragLeave={e=>e.currentTarget.classList.remove('drop')} onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('drop');onDrop(e.dataTransfer.getData(type))}}><span>{title}</span>{item?<><strong>{item.name}</strong><b>{weight(displayWeight??item.weight_g)}</b></>:<em>Hier ablegen</em>}</div>}
