...
function GearView({gear,refreshAll,setMessage}){
 const [form,setForm]=useState(emptyGear);const [editId,setEditId]=useState(null);const [q,setQ]=useState('');const [categoryFilter,setCategoryFilter]=useState('');const [locationFilter,setLocationFilter]=useState('');const [sortOrder,setSortOrder]=useState('name-asc');const [selectedIds,setSelectedIds]=useState([])
 ...
 const filtered=gear.filter(g=>`${g.name} ${g.category} ${g.location} ${g.brand} ${g.notes}`.toLowerCase().includes(q.toLowerCase())&&(!categoryFilter||g.category===categoryFilter)&&(!locationFilter||g.location===locationFilter)).sort((a,b)=>{
  if(sortOrder==='name-desc')return b.name.localeCompare(a.name,'de',{sensitivity:'base'})
  if(sortOrder==='category-asc')return (a.category||'').localeCompare(b.category||'','de',{sensitivity:'base'})||a.name.localeCompare(b.name,'de',{sensitivity:'base'})
  if(sortOrder==='location-asc')return (a.location||'').localeCompare(b.location||'','de',{sensitivity:'base'})||a.name.localeCompare(b.name,'de',{sensitivity:'base'})
  return a.name.localeCompare(b.name,'de',{sensitivity:'base'})
 })
 const visibleIds=filtered.map(g=>g.id);const allVisibleSelected=visibleIds.length>0&&visibleIds.every(id=>selectedIds.includes(id))
 async function save(e){e.preventDefault();const payload={...form,weight_g:Number(form.weight_g)||0,quantity:Math.max(1,Number(form.quantity)||1),price_chf:Number(form.price_chf)||0}
 ...
 async function del(id){if(!confirm('Artikel wirklich löschen?'))return;const r=await supabase.from('gear').delete().eq('id',id);if(r.error)setMessage(r.error.message);else{setSelectedIds(ids=>ids.filter(x=>x!==id));refreshAll()}}
 async function deleteSelected(){if(!selectedIds.length)return;if(!confirm(`${selectedIds.length} ausgewählte Artikel wirklich löschen?`))return;const r=await supabase.from('gear').delete().in('id',selectedIds);if(r.error)setMessage(r.error.message);else{setMessage(`${selectedIds.length} Artikel wurden gelöscht.`);setSelectedIds([]);await refreshAll()}}
 function toggleSelected(id){setSelectedIds(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])}
 function toggleAllVisible(){setSelectedIds(ids=>allVisibleSelected?ids.filter(id=>!visibleIds.includes(id)):[...new Set([...ids,...visibleIds])])}
 ...
 return <div className="grid"><form className="card span4" onSubmit={save}><h2>{editId?'Artikel bearbeiten':'Artikel erfassen'}</h2><Field label="Artikelname"><input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></Field><div className="two"><Field label="Kategorie"><input value={form.category} onChange={e=>setForm({...form,category:e.target.value})}/></Field><Field label="Gewicht (g)"><input type="number" min="0" value={form.weight_g} onChange={e=>setForm({...form,weight_g:e.target.value})}/></Field><Field label="Menge"><input type="number" min="1" value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})}/></Field><Field label="Lagerplatz"><input value={form.location} onChange={e=>setForm({...form,location:e.target.value})}/></Field><Field label="Hersteller"><input value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})}/></Field><Field label="Preis (CHF)"><input type="number" step="0.05" min="0" value={form.price_chf} onChange={e=>setForm({...form,price_chf:e.target.value})}/></Field></div><Field label="Notizen"><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></Field><div className="actions"><button className="primary">Speichern</button>{editId&&<button type="button" className="secondary" onClick={()=>{setEditId(null);setForm(emptyGear)}}>Abbrechen</button>}</div></form><section className="card span8"><h2>Ausrüstungslager</h2><div className="filterbar gear-filterbar"><input className="search" placeholder="Artikel, Hersteller oder Notizen suchen" value={q} onChange={e=>setQ(e.target.value)}/><select aria-label="Kategorie filtern" value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value)}><option value="">Alle Kategorien</option>{categories.map(c=><option key={c} value={c}>{c}</option>)}</select><select aria-label="Lagerplatz filtern" value={locationFilter} onChange={e=>setLocationFilter(e.target.value)}><option value="">Alle Lagerplätze</option>{locations.map(l=><option key={l} value={l}>{l}</option>)}</select><select aria-label="Artikel sortieren" value={sortOrder} onChange={e=>setSortOrder(e.target.value)}><option value="name-asc">Artikel A–Z</option><option value="name-desc">Artikel Z–A</option><option value="category-asc">Kategorie A–Z</option><option value="location-asc">Lagerplatz A–Z</option></select><button type="button" className="secondary" onClick={()=>{setQ('');setCategoryFilter('');setLocationFilter('');setSortOrder('name-asc')}}>Filter zurücksetzen</button></div><div className="selectionbar"><span>{filtered.length} von {gear.length} Artikeln · {selectedIds.length} ausgewählt</span><div className="actions"><button type="button" className="secondary" onClick={toggleAllVisible} disabled={!filtered.length}>{allVisibleSelected?'Sichtbare abwählen':'Alle sichtbaren auswählen'}</button><button type="button" className="smallbtn danger bulk-delete" onClick={deleteSelected} disabled={!selectedIds.length}>Ausgewählte löschen</button></div></div><div className="tablewrap"><table><thead><tr><th className="checkcol"><input type="checkbox" aria-label="Alle sichtbaren Artikel auswählen" checked={allVisibleSelected} onChange={toggleAllVisible}/></th><th>Artikel</th><th>Kategorie</th><th>Gewicht</th><th>Lagerplatz</th><th></th></tr></thead><tbody>{filtered.map(g=><tr key={g.id} className={selectedIds.includes(g.id)?'selected-row':''} draggable onDragStart={e=>e.dataTransfer.setData('gearId',g.id)}><td className="checkcol"><input type="checkbox" aria-label={`${g.name} auswählen`} checked={selectedIds.includes(g.id)} onChange={()=>toggleSelected(g.id)} onClick={e=>e.stopPropagation()}/></td><td><strong>{g.name}</strong><small>{g.brand}</small></td><td>{g.category||'–'}</td><td>{weight(g.weight_g*g.quantity)}<small>{g.quantity} × {weight(g.weight_g)}</small></td><td>{g.location||'–'}</td><td><button className="smallbtn" onClick={()=>edit(g)}>Bearbeiten</button><button className="smallbtn danger" onClick={()=>del(g.id)}>Löschen</button></td></tr>)}</tbody></table></div>{!filtered.length&&<Empty/>}</section></div>
}

function ListsView({gear,lists,listItems,refreshAll,setMessage}){
 const [name,setName]=useState('');const [desc,setDesc]=useState('');const [q,setQ]=useState('');const [selectedGear,setSelectedGear]=useState({})
 ...
 async function add(listId,gearId){if(!gearId)return;const existing=listItems.find(x=>x.pack_list_id===listId&&x.gear_id===gearId);const r=existing?await supabase.from('pack_list_items').update({quantity:existing.quantity+1}).eq('id',existing.id):await supabase.from('pack_list_items').insert({pack_list_id:listId,gear_id:gearId,quantity:1});if(r.error)setMessage(r.error.message);else{setSelectedGear(v=>({...v,[listId]:''}));refreshAll()}}
 async function remove(id){await supabase.from('pack_list_items').delete().eq('id',id);refreshAll()}
 async function deleteList(id){if(confirm('Packliste löschen?')){await supabase.from('pack_lists').delete().eq('id',id);refreshAll()}}
 const filteredGear=gear.filter(g=>`${g.name} ${g.category||''} ${g.location||''}`.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>a.name.localeCompare(b.name,'de'))
 return <div className="grid"><aside className="card span4"><form onSubmit={create}><h2>Neue Packliste</h2><Field label="Name"><input required value={name} onChange={e=>setName(e.target.value)}/></Field><Field label="Beschreibung"><textarea value={desc} onChange={e=>setDesc(e.target.value)}/></Field><button className="primary">Erstellen</button></form><hr/><h3>Artikel ziehen</h3><input className="search" placeholder="Artikel suchen" value={q} onChange={e=>setQ(e.target.value)}/><div className="drag-list">{filteredGear.map(g=><div draggable onDragStart={e=>e.dataTransfer.setData('gearId',g.id)} className="drag-item" key={g.id}><strong>{g.name}</strong><small>{weight(g.weight_g)} · {g.category||'Ohne Kategorie'} · {g.location||'Kein Lagerplatz'}</small></div>)}</div></aside><section className="span8">{lists.map(l=>{const items=listItems.filter(i=>i.pack_list_id===l.id);const total=items.reduce((s,i)=>s+(gear.find(g=>g.id===i.gear_id)?.weight_g||0)*i.quantity,0);return <article className="card listcard" key={l.id} draggable onDragStart={e=>e.dataTransfer.setData('listId',l.id)} onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('drop')}} onDragLeave={e=>e.currentTarget.classList.remove('drop')} onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('drop');add(l.id,e.dataTransfer.getData('gearId'))}}><div className="listhead"><div><h2>{l.name}</h2><p>{l.description}</p></div><div><b>{weight(total)}</b><button className="smallbtn danger" onClick={()=>deleteList(l.id)}>Löschen</button></div></div><div className="packlist-add-controls" onDragStart={e=>e.stopPropagation()}><Field label="Artikel per Dropdown hinzufügen"><select value={selectedGear[l.id]||''} onChange={e=>setSelectedGear(v=>({...v,[l.id]:e.target.value}))}><option value="">Artikel auswählen</option>{gear.slice().sort((a,b)=>a.name.localeCompare(b.name,'de')).map(g=><option key={g.id} value={g.id}>{g.name} – {weight(g.weight_g)} – {g.category||'ohne Kategorie'} – {g.location||'kein Lagerplatz'}</option>)}</select></Field><button type="button" className="primary" disabled={!selectedGear[l.id]} onClick={()=>add(l.id,selectedGear[l.id])}>Hinzufügen</button></div><div className="drop-hint">Oder Artikel hier per Drag-and-drop ablegen</div><div className="tablewrap"><table><thead><tr><th>Artikel</th><th>Menge</th><th>Gewicht</th><th></th></tr></thead><tbody>{items.map(i=>{const g=gear.find(x=>x.id===i.gear_id);return g?<tr key={i.id}><td>{g.name}<small>{g.location||'Kein Lagerplatz'}</small></td><td>{i.quantity}</td><td>{weight(g.weight_g*i.quantity)}</td><td><button className="smallbtn danger" onClick={()=>remove(i.id)}>Entfernen</button></td></tr>:null})}</tbody></table></div>{!items.length&&<Empty/>}</article>})}{!lists.length&&<div className="card"><Empty/></div>}</section></div>
}
...
function CompareView({gear,lists,listItems}){
 ...
 return <div className="stack"><section className="card"><h2>Artikel vergleichen</h2><p className="muted">Artikel per Drag-and-drop ablegen oder bequem über die Dropdowns auswählen.</p><div className="compare-selects"><Field label="Artikel A auswählen"><select value={a} onChange={e=>setA(e.target.value)}><option value="">Bitte auswählen</option>{gear.map(g=><option key={g.id} value={g.id}>{g.name} – {weight(g.weight_g)} – {g.location||'kein Lagerplatz'}</option>)}</select></Field><Field label="Artikel B auswählen"><select value={b} onChange={e=>setB(e.target.value)}><option value="">Bitte auswählen</option>{gear.map(g=><option key={g.id} value={g.id}>{g.name} – {weight(g.weight_g)} – {g.location||'kein Lagerplatz'}</option>)}</select></Field></div><DropCompare title="Artikel A" item={itemA} onDrop={setA} type="gearId"/><DropCompare title="Artikel B" item={itemB} onDrop={setB} type="gearId"/>{itemA&&itemB&&<Result a={itemA.weight_g} b={itemB.weight_g} an={itemA.name} bn={itemB.name}/>}<h3 className="drag-title">Artikel zum Ziehen</h3><div className="drag-grid">{gear.map(g=><div className="drag-item" draggable onDragStart={e=>e.dataTransfer.setData('gearId',g.id)} key={g.id}><strong>{g.name}</strong><small>{weight(g.weight_g)} · {g.location||'Kein Lagerplatz'}</small></div>)}</div>{!gear.length&&<Empty/>}</section><section className="card"><h2>Packlisten vergleichen</h2><p className="muted">Packlisten per Drag-and-drop ablegen oder über die Dropdowns auswählen.</p><div className="compare-selects"><Field label="Packliste A auswählen"><select value={la} onChange={e=>setLa(e.target.value)}><option value="">Bitte auswählen</option>{lists.map(l=><option key={l.id} value={l.id}>{l.name} – {weight(listWeight(l.id))}</option>)}</select></Field><Field label="Packliste B auswählen"><select value={lb} onChange={e=>setLb(e.target.value)}><option value="">Bitte auswählen</option>{lists.map(l=><option key={l.id} value={l.id}>{l.name} – {weight(listWeight(l.id))}</option>)}</select></Field></div><DropCompare title="Packliste A" item={listA} onDrop={setLa} type="listId" displayWeight={la?listWeight(la):null}/><DropCompare title="Packliste B" item={listB} onDrop={setLb} type="listId" displayWeight={lb?listWeight(lb):null}/>{listA&&listB&&<Result a={listWeight(la)} b={listWeight(lb)} an={listA.name} bn={listB.name}/>}<h3 className="drag-title">Packlisten zum Ziehen</h3><div className="drag-grid">{lists.map(l=><div className="drag-item" draggable onDragStart={e=>e.dataTransfer.setData('listId',l.id)} key={l.id}><strong>{l.name}</strong><small>{weight(listWeight(l.id))}</small></div>)}</div>{!lists.length&&<Empty/>}</section></div>
}
